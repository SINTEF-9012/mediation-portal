...
class TestPrinter extends SpecificationWithJUnit {
   ...
   val ClassB = new Class("CB", false, thePackage)
   val featureE = new Feature(Some(ClassB), "featureE", Any, 0, Some(1), false, false, Some(featureA)) 
   ...
   "A printer" should {
      ...
      "print features having an opposite properly" in {
         val expected = "featureE: Any [0..1] ~ test.MyClass.featureA"
         val output = featureE.accept(printer, new StringBuilder())
         output.result must beEqualTo(expected)
      }
      ...
      "print the Any type properly" in {
         val input = new StringBuilder()
         val output = Any.accept(printer, input)
         output.result must beEqualTo("Any")
      }
   }
