...
class Runner(partners: PartnerHandler) extends HttpSpraySupport {
  ...
  /**
   * Define the URL of the mapping repository
   */
  private[this] val MAPPING_REPOSITORY_URL = "/mediation/repositories/mappings"
     
  /**
   * Define the URL of the models repository
   */
  ...
  private[this] def fetch(modelId: String): Option[Model] = {
    ...
    val result = conduit.pipeline(Get(MODEL_REPOSITORY_URL + "/" + modelId, None))
    try {
  ...
  private[this] def publishMapping(mapping: Mapping): String = {
    ...
    val result = conduit.pipeline(Post("/mediation/repositories/mappings", None))
